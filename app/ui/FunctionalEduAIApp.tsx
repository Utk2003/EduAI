...
type DocumentRole = "Question paper"|"Model answer / marking scheme"|"Ungraded answer sheet"|"Teacher-graded answer sheet"|"Supporting reference";
...
function Work({state,setState,selected,openAssessment,open,update,notify}:any){
  ...
  const uploadDirectly=()=>{const id=`a${Date.now()}`;const a:Assessment={id,title:"Uploaded evidence bundle",type:"Evidence upload",grade:"",section:"",subject:"To be detected",maxMarks:1,date:new Date().toISOString().slice(0,10),stage:"draft",files:[],questions:0,reviewed:0,totalReviews:0,quality:0,published:false,version:1};setState((s:DemoState)=>({...s,assessments:[a,...s.assessments],events:["Evidence bundle started without assessment setup",...s.events]}));openAssessment(id,"Work");window.setTimeout(()=>open("upload"),0)};
  return <><PageHead eyebrow="Teacher workspace" title="Work & evidence" subtitle="Upload evidence immediately, or create assessment details when you need them."><button className="primary" onClick={uploadDirectly}>↑ Upload evidence</button><button className="secondary" onClick={()=>open("create-assessment")}>＋ Optional assessment details</button></PageHead>
    <section className="card span-2 evidence-entry"><div><p className="eyebrow">Assessment setup is optional</p><h2>Start with the documents already on your desk</h2><p>Add a question paper, model answers or marking scheme, and scanned handwritten or digital answer sheets. Classify every upload so grading and downstream resources use the correct evidence.</p></div><div className="evidence-role-list"><span>Question paper</span><span>Model answers</span><span>Ungraded answers</span><span>Teacher-graded answers</span></div></section>
...
function PerFileGradeDialog({assessment,file,state,setState,update,open,notify}:any){
  ...
  const modelAnswers=candidates.filter(f=>f.documentRole==="Model answer / marking scheme"||(!f.documentRole&&/answer.?key|model.?answer|marking.?scheme|solutions?/i.test(f.name)));
  const preferredQP=questionPapers[0];
  const preferredKey=modelAnswers[0];
  ...
  return <><DialogHead eyebrow={assessment.title} title={alreadyGraded?"Regrade answer sheet":"Grade answer sheet"}/>
    ...
    <Field label="Grade against question paper">
      <select value={qpId} onChange={e=>setQpId(e.target.value)}>
        ...
        {questionPapers.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
      </select>
    </Field>
    <Field label="Answer key / marking scheme (recommended)">
      <select value={answerKeyId} onChange={e=>setAnswerKeyId(e.target.value)}>
        ...
        {modelAnswers.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}
      </select>
    </Field>
...
function WorksheetDialog({setState,worksheet,sourceAssessment,sourceResult,presetConcept,presetTitle,presetStudent,done}:any){const concept=worksheet?.concept||presetConcept||"the target concept";const [generated,setGenerated]=useState(Boolean(worksheet?.content));const [generating,setGenerating]=useState(false);const [genError,setGenError]=useState("");const [content,setContent]=useState<WorksheetContent|null>(worksheet?.content||null);const [template,setTemplate]=useState(worksheet?.template||"Guided recovery");const [mcq,setMcq]=useState(worksheet?.mcq??6);const [subjective,setSubjective]=useState(worksheet?.subjective??4);const [difficulty,setDifficulty]=useState(worksheet?.difficulty||"Mixed");const [title,setTitle]=useState(worksheet?.title||presetTitle||`${concept} Practice`);const save=()=>{const resource:Worksheet={id:worksheet?.id||`r${Date.now()}`,title,type:"Targeted worksheet",status:"Approved",template,concept,mcq,subjective,difficulty,answerSheets:worksheet?.answerSheets||0,gradedSheets:worksheet?.gradedSheets||0,content:content||worksheet?.content};setState((s:DemoState)=>({...s,resources:worksheet?s.resources.map(r=>r.id===worksheet.id?resource:r):[resource,...s.resources],events:[`Worksheet saved · ${resource.title}`,...s.events]}));done()};
  const generate=async()=>{
    ...
    try{
      ...
      const res=await authFetch("/api/generate-worksheet",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({concept,subject:sourceAssessment?.subject||worksheet?.subject,difficulty,template,mcqCount:mcq,subjectiveCount:subjective,evidenceSummary})});
      const payload=await res.json();
      logApiTiming(setState,payload?.timing);
...
function inferDocumentRole(name:string):DocumentRole{
  if(/model.?answer|answer.?key|marking.?scheme|solution/i.test(name))return "Model answer / marking scheme";
...
function UploadDialogV2({assessment,update,done}:any){
  ...
  return <><DialogHead eyebrow={assessment.title} title="Upload and classify evidence"/><p className="modal-copy">Assessment details are optional. The document roles below determine how grading, learning-gap analysis, study guides and worksheets use each file.</p><div className="dropzone" role="button" tabIndex={0} onClick={()=>input.current?.click()} onKeyDown={e=>(e.key==="Enter"||e.key===" ")&&input.current?.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();add(e.dataTransfer.files)}}><span>↑</span><b>Drop scanned handwriting or digital files</b><small>PDF, JPG, PNG, HEIC, DOCX, XLSX or CSV · multiple files · 10 MB each</small><button type="button" className="secondary" onClick={e=>{e.stopPropagation();input.current?.click()}}>Browse / Choose File</button><input ref={input} className="file-input" type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.heic,.docx,.xlsx,.csv" onChange={e=>e.target.files&&add(e.target.files)}/></div>{error&&<p className="form-error" role="alert">{error}</p>}<div className="upload-list evidence-upload-list">{files.map(f=><div key={f.id}>{f.preview?<img src={f.preview} alt={`Preview ${f.name}`}/>:<span className="file-icon">{f.name.split(".").pop()?.toUpperCase()}</span>}<div><b>{f.name}</b><small>{(f.size/1024/1024).toFixed(2)} MB · {f.status}</small><select aria-label={`Document role for ${f.name}`} value={f.documentRole||inferDocumentRole(f.name)} onChange={e=>setFiles(items=>items.map(item=>item.id===f.id?{...item,documentRole:e.target.value as DocumentRole}:item))}>{roles.map(role=><option key={role}>{role}</option>)}</select><Progress value={f.progress}/></div><button disabled={uploading} onClick={()=>setFiles(x=>x.filter(v=>v.id!==f.id))} aria-label={`Remove ${f.name}`}>×</button></div>)}</div><div className="button-row">{uploading&&<button className="secondary" onClick={pause}>{paused?"Resume":"Pause"}</button>}<button className="secondary" disabled={uploading} onClick={()=>setFiles(fs=>fs.map(f=>f.status.includes("Failed")?{...f,status:"Ready",progress:0}:f))}>Retry failed</button><button className="primary" disabled={uploading} onClick={start}>Save evidence & start OCR</button></div></>
}
